@extends('sarana.layouts.master')

@section('title', 'Data Pengadaan')
@section('judul', 'Data Pengadaan')
@section('content')
    <div class="container-fluid">
        <div class="card card-default">
            <div class="card-header">{{ __('Pengelolaan Pengadaan') }}</div>
            <div class="card-body">
                <table id="table-data" class="table table-striped table-white ">
                    <thead>
                        <tr class="text-center">
                            <th>NO</th>
                            <th>Kode Pengadaan</th>
                            <th>Tanggal Pengajuan</th>
                            <th>Ruangan</th>
                            <th>Total</th>
                            <th>Keterangan</th>
                            <th>AKSI</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $no=1; @endphp
                        @foreach ($pengadaan as $row)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>{{ $row->kode}}</td>
                                <td>{{ $row->tanggal_pengadaan}}</td>
                                <td>{{ $row->ruangan->nama_ruangan }}</td>
                                <td>{{ $row->harga }}</td>
                                <td>{{ $row->keterangan }}</td>
                               
                                
                                <td>
                                    @if($row->keterangan == "Barang Sudah Di Distribusikan")
                                    <a href="pengadaan/detail/{{$row->id}}" class="btn btn-info">Detail</a>
                                    @else
                                    @foreach ($pengadaan_detail as $row1)
                                    @if($row->id == $row1->pengadaan_id && $row1->count()>0)
                                           <a href="pengadaan/distribusikan/{{$row->id}}" class="btn btn-success">Distribusi</a>
                                           
                                           @endif
                                           @endforeach
                                           <a href="pengadaan/detail/{{$row->id}}" class="btn btn-info">Detail</a>
                                           @endif
                                          
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                    <tfoot>
                        <tr>
                            <td>Jumlah</td>
                            <td colspan="2"></td>
                            <td>Rp.{{$total}}</td>
                            <td>Sisa</td>
                            <td colspan="1"></td>
                            <td>Rp.{{$sisa}}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        </div>
    </div>

    @stop

    @section('js')
    <script src="//cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <script>
$(function() {
            $(document).on('click', '#btn-edit-jadwal', function() {
                let id = $(this).data('id');
                $.ajax({
                    type: "get",
                    url: "{{ url('sarana/ajaxadmin/dataPerencanaan') }}/" + id,
                    dataType: 'json',
                    success: function(res) {
                        $('#edit-tanggal').val(res.tanggal_perencanaan);
                        $('#edit-ruangan').val(res.ruangan_id);
                        $('#edit-status').val(res.status);
                        $('#edit-keterangan').val(res.keterangan);
                        $('#edit-kode').val(res.kode);
                        $('#edit-id').val(res.id);
                    },
                });
            });
        });

        @if(session('status'))
            Swal.fire({
                title: 'Congratulations!',
                text: "{{ session('status') }}",
                icon: 'Success',
                timer: 3000
            })
        @endif
        @if($errors->any())
            @php
                $message = '';
                foreach($errors->all() as $error)
                {
                    $message .= $error."<br/>";
                }
            @endphp
            Swal.fire({
                title: 'Error',
                html: "{!! $message !!}",
                icon: 'error',
            })
        @endif
        
        function deleteConfirmation(npm, judul) {
            swal.fire({
                title: "Hapus?",
                type: 'warning',
                text: "Apakah anda yakin akan menghapus data dengan kode " + judul + "?!",

                showCancelButton: !0,
                confirmButtonText: "Ya, lakukan!",
                cancelButtonText: "Tidak, batalkan!",
                reverseButtons: !0
            }).then(function(e) {

                if (e.value === true) {
                    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');

                    $.ajax({
                        type: 'POST',
                        url: "perencanaan/delete/" + npm,
                        data: {
                            _token: CSRF_TOKEN
                        },
                        dataType: 'JSON',
                        success: function(results) {
                            if (results.success === true) {
                                swal.fire("Selamat", results.message, "success");
                                // refresh page after 2 seconds
                                setTimeout(function() {
                                    location.reload();
                                }, 1000);
                            } else {
                                swal.fire("Error!", results.message, "error");
                            }
                        }
                    });

                } else {
                    e.dismiss;
                }

            }, function(dismiss) {
                return false;
            })
        }


    </script>
    @stop